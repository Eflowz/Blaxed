  ...
  <div key={item._id} className="flex items-center justify-between py-4">
    ...
    >
      <FaTrashAlt size={20} />
          ...
          <div className="cart-summary mt-6 p-4 bg-gray-100 rounded-lg shadow-md">
            <h3 className="text-xl font-semibold">Cart Summary</h3>
            <p className="text-lg">Total Items: {getTotalItems()}</p>
            <p className="text-lg font-semibold">
              Total Price: {currency}{cart.reduce((total, item) => total + item.totalPrice, 0).toFixed(2)}
            </p>
            <button className="mt-4 px-4 py-2 bg-green-500 text-white rounded-md shadow hover:bg-green-600">
              Proceed to Checkout
            </button>
          </div>
        </div>
