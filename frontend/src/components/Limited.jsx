...
//         <div className="mb-4">
//             <p className="text-gray-500 line-through">${product.price.toFixed(2)}</p> {/* Original price */}
//             <p className="text-red-500  font-bold">${discountedPrice}</p> {/* Discounted price */}
//         </div>

//         <button
//             onClick={() => addToCart(product._id, product.name, discountedPrice, product.image)}
...
//         </div>
//         <button
//             onClick={() => addToCart(selectedProduct._id, selectedProduct.name, selectedProduct.price, selectedProduct.image)}
    ...
    return (
        <div className="limited-offer mt-5">
            ...
            <div className="container mx-auto">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {limitedProducts.map((product, index) => {
                        const discountedPrice = parseFloat((product.price * 0.6).toFixed(2)); // Ensure formatted discounted price
                        ...
                        return (
                            <div key={index} className="product bg-white p-4 shadow-md rounded-lg text-left">
                                ...
                                {/* Price Section with Discount */}
                                <div className="mb-4">
                                    <p className="text-gray-500 line-through">${parseFloat(product.price).toFixed(2)}</p> {/* Original price */}
                                    <p className="text-red-500 font-bold">${discountedPrice}</p> {/* Discounted price */}
                                </div>
                ...
                {selectedProduct && (
                    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                        <div className="bg-white p-6 rounded-lg w-full max-w-md relative">
                            ...
                            <button
                                onClick={() => addToCart(selectedProduct._id, selectedProduct.name, parseFloat(selectedProduct.price).toFixed(2), selectedProduct.image)}
                                className="bg-green-500 text-white py-2 px-4 rounded w-full"
                            >
                                Add to cart
                            </button>
